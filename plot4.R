...
##Generating the plot4 charts and png file
data.set$Global_active_power <- as.numeric(data.set$Global_active_power)
data.set$Global_reactive_power <- as.numeric(data.set$Global_reactive_power)
...
data.set$Voltage <- as.numeric(data.set$Voltage)

#Open png device
png(file="plot4.png")

#setting mfrow parameter to generate 2 rows of 2 plots
par(mfrow = c(2,2))
